/*
Pour créer un graphe ajouter la ligne:
let graph=new Graph(10)
En remplaçant 10 par votre nombre
Pour simplifier un graphe:
graph.simplify()
Pour afficher un graphe:
console.log(graph.to_String())
Pour comparer deux graphes:
console.log(Graph.Print_graph_dif(Graph.Get_dif_between(graph,graph2)))
NB: Il est préférable d'utiliser la méthode: graph.clean()  sur les deux graphes avant de les comparer pour supprimer les points non relié
